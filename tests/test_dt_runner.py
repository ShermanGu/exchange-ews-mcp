...
def test_unified_dt_suite_groups_atomic_and_workflow(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        dt_runner_module,
        "run_template_v06_integration_tests",
        lambda *args, **kwargs: _fake_group_report("template_step"),
    )
    report = run_dt_suite(
    ...
    assert [group["id"] for group in report["groups"]] == [
        "atomic", "workflow-v03", "semantic-mail-v04", "calendar-v05", "template-mail-v06"
    ]
    assert Path(report["report_path"]).exists()

...
def list_emails(
    ...
    folder 标准值：inbox、drafts、sentitems、deleteditems、junkemail、outbox。
    兼容 Outlook 显示名，例如 Sent Items、Deleted Items、Junk Email。
    """
...
def search_emails(
    ...
    folder/folders 标准值：inbox、drafts、sentitems、deleteditems、junkemail、outbox。
    同时兼容 Outlook 显示名，例如 Sent Items 会自动转换为 sentitems。
    设置 folders 时忽略 folder。participant_contains 会匹配发件人、To 或 CC；多个条件使用 AND。
    """
...
def update_draft(
    ...
    importance 标准值为 Low/Normal/High；同时兼容大小写和 High Importance 等写法。
    """
...
def compose_email(
    ...
    普通邮件直接传完整 body_html。使用历史模板时，传 template_ref，并让
    body_html 只包含新的正文片段；服务器会在保存的完整模板中填入新内容，
    避免 Agent 因工具结果长度限制拿到截断 HTML。
    系统只复制最终 HTML 仍引用的 cid 内联图片。普通模板附件默认不复制。
    """
...
def extract_email_template(
    ...
    """从一封现有邮件提取可复用 HTML 模板，但不创建草稿。

    优先使用 Exchange EWS UniqueBody（当前会话项独有正文）只读取顶部第一封，排除引用历史；
    只有服务器未返回 UniqueBody 时才回退识别 Outlook/Gmail HTML 分隔结构。
    完整模板保存在 template_ref。较大模板只返回预览与字符数，后续应把
    template_ref + 仅含新正文片段的 body_html 交给 compose_email 或 reply_to_email。
    folders 支持 Sent Items 等 Outlook 显示名；多个候选时返回 resume_token。
    """
    return service.extract_email_template(
        message_ref=message_ref,
        folders=folders,
...
def find_email(
    ...
    folders 使用标准值 inbox/drafts/sentitems/deleteditems/junkemail/outbox，
    也兼容 Sent Items 等 Outlook 显示名。主 Agent 负责把“上周”“昨天”等自然语言转换为 after/before。
...
def reply_to_email(
    ...
    """定位原邮件并创建回复或全部回复草稿，绝不发送。

    普通回复直接传完整 body_html。使用历史格式时，传 template_ref，并让
    body_html 只包含新的回复正文片段，由服务器在完整模板中填入新内容。回复目标仍由
    message_ref 或搜索条件独立决定，模板不会改变 Reply/Reply-All 收件人。
    folders 支持 sentitems 和 Sent Items 等 Outlook 显示名。
    """
...
def forward_email(
    ...
    folders 标准值为 inbox/drafts/sentitems/deleteditems/junkemail/outbox；
    也接受 Sent Items、Deleted Items 等 Outlook 显示名。
    """
...
def get_user_availability(
    ...
    attendee_type 标准值为 Organizer/Required/Optional/Room/Resource；
    兼容小写、空格写法和常见别名，例如 meeting room。
    """
...
def main() -> None:
    """Start the production MCP server with the curated 18-tool surface."""
    mcp.run(transport="stdio")


def debug_main() -> None:
    """Start the full 24-tool MCP server for development and troubleshooting."""

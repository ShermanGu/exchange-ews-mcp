...
def compose_email_command(args: argparse.Namespace) -> int:
    _print_json(service.compose_email(
        ...
        copy_template_inline_images=not args.no_template_inline_images,
        copy_template_attachments=args.copy_template_attachments,
        lookback_days=args.lookback_days,
    ))
    return 0


def extract_email_template_command(args: argparse.Namespace) -> int:
    _print_json(service.extract_email_template(
        message_ref=args.message_ref,
        folders=_split_values(args.folders) or None,
    ...
    _print_json(
        {
            ...
            "groups": ["atomic", "workflow-v03", "semantic-mail-v04", "calendar-v05", "template-mail-v06"],
        }
    )
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--template-ref")
    p.add_argument("--no-template-inline-images", action="store_true")
    p.add_argument("--copy-template-attachments", action="store_true")
    p.add_argument("--lookback-days", type=int, default=365)
    _add_html_body(p)
    p.set_defaults(func=compose_email_command)

    p = sub.add_parser("extract-email-template", help="从现有邮件提取 HTML 模板，不创建草稿")
    ...
    p.add_argument("--limit", type=int, default=20)
    p.add_argument("--lookback-days", type=int, default=365)
    p.set_defaults(func=extract_email_template_command)
    ...
    p.add_argument(
        "--group",
        action="append",
        choices=["atomic", "workflow-v03", "semantic-mail-v04", "calendar-v05", "template-mail-v06"],
    ...
    p.add_argument("--debug-tools", action="store_true", help="输出完整 23 工具的调试 server 配置")
    p.set_defaults(func=mcp_config)

    p = sub.add_parser("serve", help="启动 stdio MCP server")
    p.add_argument("--debug-tools", action="store_true", help="暴露完整 24 个底层与高层工具")

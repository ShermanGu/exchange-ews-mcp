...
def _folder_alias_key(value: str) -> str:
    normalized = unicodedata.normalize("NFKC", value).strip().casefold()
    return re.sub(r"[\s_\-]+", "", normalized)


def normalize_mail_folder(folder: str) -> str:
    """Normalize one EWS/Outlook folder input to a distinguished-folder id."""
    if not isinstance(folder, str):
        raise ValueError("邮箱文件夹必须是字符串。")
    raw = unicodedata.normalize("NFKC", folder).strip()
    if not raw:
        raise ValueError("邮箱文件夹不能为空。")
    normalized = _FOLDER_ALIASES.get(_folder_alias_key(raw))
    if normalized is None:
        allowed = ", ".join(sorted(ALLOWED_FOLDERS))
        raise ValueError(
            f"不支持的邮箱文件夹 {folder!r}。可使用标准值 {allowed}，"
            "也可使用常见中英文 Outlook 名称，例如 收件箱/inbox、"
            "已发送邮件/sent items、草稿箱/drafts。"
        )
    return normalized


def normalize_mail_folders(
    *,
    folder: str | None = None,
    folders: list[str] | tuple[str, ...] | str | None = None,
    default_folder: str = "inbox",
) -> list[str]:
    """Apply search_emails-style folder/folders semantics and deduplicate.

    ``folders`` takes precedence when supplied.  A single string is accepted as
    a defensive convenience for direct Python callers; the MCP schema still
    advertises a list for ``folders`` and a scalar ``folder``.
    """
    selected: list[str]
    if folders is not None:
        selected = [folders] if isinstance(folders, str) else list(folders)
    else:
        selected = [folder if folder is not None else default_folder]
    if not selected:
        raise ValueError("folders 至少需要一个邮箱文件夹。")
    normalized: list[str] = []
    for value in selected:
        canonical = normalize_mail_folder(value)
...
class EwsClient:
    ...
    def get_email(
        ...
        *,
        ...
        max_body_chars: int | None = 50000,
    ) -> dict[str, Any]:
        if not item_id.strip():
            raise ValueError("item_id 不能为空。")
        if max_body_chars is not None and not 1000 <= max_body_chars <= 500000:
            raise ValueError("max_body_chars 必须为 None，或在 1000 到 500000 之间。")
        ...
        unique_body_html = (
            unique_body.text if unique_body is not None and unique_body.text is not None else ""
        )
        ...
        unique_body_server_truncated = (
            str(unique_body.attrib.get("IsTruncated") or "").casefold() == "true"
            if unique_body is not None
            else False
        )
        body_local_truncated = (
            max_body_chars is not None and len(body_html) > max_body_chars
        )
        unique_body_local_truncated = (
            max_body_chars is not None and len(unique_body_html) > max_body_chars
        )
        result["body_type"] = body.attrib.get("BodyType") if body is not None else None
        ...
        result["body_html"] = (
            body_html if max_body_chars is None else body_html[:max_body_chars]
        )
        ...
        result["unique_body_server_truncated"] = unique_body_server_truncated
        result["unique_body_local_truncated"] = unique_body_local_truncated
        result["unique_body_html"] = (
            unique_body_html
            if max_body_chars is None
            else unique_body_html[:max_body_chars]
        )
        result["unique_body_truncated"] = (
            unique_body_server_truncated or unique_body_local_truncated
        )
        return result

    def get_file_attachment(self, *, attachment_id: str) -> dict[str, Any]:
        normalized = attachment_id.strip()
        if not normalized:
            raise ValueError("attachment_id 不能为空。")
        payload = build_get_attachment_request(
            exchange_version=self.config.exchange_version,
        ...
        root = self._parse_xml(response)
        self._raise_for_ews_error(root)
        attachment = root.find(f".//{q(TYPES_NS, 'FileAttachment')}")
        if attachment is None:
            raise EwsError("GetAttachment 响应中没有 FileAttachment。")
        content_text = _text(attachment, "Content") or ""
        try:
            content = base64.b64decode(re.sub(r"\s+", "", content_text), validate=True)
        except Exception as exc:
            raise EwsError("GetAttachment 返回了无效的 Base64 内容。") from exc
        attachment_identity = attachment.find(q(TYPES_NS, "AttachmentId"))
        return {
            "attachment_id": (
                attachment_identity.attrib.get("Id")
                if attachment_identity is not None
                else normalized
            ),
            "filename": _text(attachment, "Name") or "attachment.bin",
            "content_type": _text(attachment, "ContentType") or "application/octet-stream",
            "content_id": _text(attachment, "ContentId"),
            "is_inline": _bool_text(attachment, "IsInline") is True,
            "content": content,
            "size": len(content),
        }
    ...
    def update_draft(
        ...
        *,
        ...
        if importance is not None:
            ...
            importance_value = mapping.get(importance.strip().lower())
    ...
    def add_file_attachment_bytes_to_draft(
        self,
        *,
        item_id: str,
        filename: str,
        ...
        if not item_id.strip():
            raise ValueError("item_id 不能为空。")
        if not filename.strip() or any(char in filename for char in "\\/\0"):
    ...
    def add_attachment_to_draft(
        ...
        *,
        ...
        if not filename or any(char in filename for char in "\\/\0"):
            raise ValueError("attachment_name 不是有效文件名。")

...
from .input_normalization import (
    normalize_availability_attendees,
...
def list_emails(**kwargs: Any) -> dict[str, Any]:
    kwargs["folder"] = normalize_mail_folder(str(kwargs.get("folder") or "inbox"))
...
def search_emails(*, folders: list[str] | None = None, **kwargs: Any) -> dict[str, Any]:
    ...
    if folders:
        ...
        result = client.search_emails_multi_folder(
            folders=normalize_mail_folders(folders), **kwargs
        )
    else:
        kwargs["folder"] = normalize_mail_folder(str(kwargs.get("folder") or "inbox"))
...
def extract_email_template(
    *,
    ...
    return configured_workflow().extract_email_template(
        message_ref=message_ref,
        folders=normalize_mail_folders(folders) if folders else None,
...
def get_user_availability(
    *,
    ...
    normalized_attendees = normalize_availability_attendees(attendees)

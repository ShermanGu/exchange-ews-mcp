...
def run_template_v06_integration_tests(
    client: EwsClient,
    config: DtTestConfig,
    *,
    app_config: AppConfig,
    ...
    """Validate v0.6 source-email templating without sending mail."""
    profile = config.normalized()
    marker = f"EWS-MCP-v{__version__}-{stamp}"
    workflow = SemanticMailWorkflow(client, store, app_config)
    steps: list[dict[str, Any]] = []
    ...
    def locate_source() -> dict[str, Any]:
        result = workflow.find_email(
            folders=["sentitems"],
            subject_contains=profile.subject_contains,
            limit=profile.search_limit,
        ...
        if not items:
            raise RuntimeError("Sent Items 中没有可用作模板的邮件。")
        source = items[0]
        source_holder.update(source)
        return {
            "returned": len(items),
            "selected_message_ref": source.get("message_ref"),
            "selected_subject": source.get("subject"),
            "selected_sent_at": source.get("sent_at"),
        }

    run_step("locate_sent_template_email", locate_source)

    def inspect_source() -> dict[str, Any]:
        if not source_holder.get("message_ref"):
            raise RuntimeError("模板来源未成功定位。")
        result = workflow.extract_email_template(
            message_ref=str(source_holder["message_ref"]),
        )
        if result.get("status") != "template_extracted":
            raise RuntimeError(f"模板提取失败：{result.get('status')}")
        if int(result.get("template_html_chars") or 0) <= 0:
            raise RuntimeError("模板提取结果没有 HTML。")
        template_holder.update(result)
        suggested = result.get("suggested_compose_inputs") or {}
        if not (suggested.get("to_queries") or suggested.get("cc_queries")):
            raise RuntimeError("模板没有可复用的 SMTP To/CC 建议。")
        return {
            "template_ref": result.get("template_ref"),
            "source_subject": (result.get("source_message") or {}).get("subject"),
            "template_chars": result.get("template_html_chars"),
            "template_preview_truncated": result.get("template_html_preview_truncated"),
            "unique_body_available": result.get("unique_body_available"),
            "quoted_history_excluded": result.get("quoted_history_excluded"),
            "history_boundary_strategy": result.get("history_boundary_strategy"),
            "to_count": len(suggested.get("to_queries") or []),
            "cc_count": len(suggested.get("cc_queries") or []),
            "inline_images_available": result.get("inline_images_available"),
        }

    run_step("extract_email_template", inspect_source)

    if read_only:
        steps.append({
            "name": "compose_email_with_template_ref",
            "status": "SKIP",
            "details": "read_only=true；未创建模板化新邮件草稿。",
        })
        steps.append({
            "name": "reply_to_email_with_template_ref",
            "status": "SKIP",
            "details": "read_only=true；未创建模板化回复草稿。",
        })
    else:
        def compose_with_template() -> dict[str, Any]:
            suggested = template_holder.get("suggested_compose_inputs") or {}
            to_queries = list(suggested.get("to_queries") or [])
            cc_queries = list(suggested.get("cc_queries") or [])
            if not to_queries and not cc_queries:
                to_queries = [profile.draft_recipient]
            result = workflow.compose_email(
                to_queries=to_queries,
                cc_queries=cc_queries,
                subject=f"[{marker}] decoupled template draft",
                template_ref=str(template_holder["template_ref"]),
                body_html=(
                    f"<h2>{marker} template compose</h2>"
                    "<p>Server-side rendering from template_ref.</p>"
                    "<img src='cid:c1'>"
                ),
            ...
            if result.get("sent") is not False:
                raise RuntimeError("模板化新邮件草稿的 sent 必须为 false。")
            draft = result["draft"]
            created_drafts.append({
                "draft_type": "compose_email_template",
                "draft_ref": draft.get("draft_ref"),
                "item_id": draft.get("item_id"),
            })
            return {
                ...
                "subject": draft.get("subject"),
                "template_ref": result.get("template_ref"),
                "inline_images_copied": result.get("template_inline_images_copied"),
                "attachments_copied": result.get("template_attachments_copied"),
                "sent": result.get("sent"),
            }

        run_step("compose_email_with_template_ref", compose_with_template)

        def reply_with_template() -> dict[str, Any]:
            if not source_holder.get("message_ref"):
                raise RuntimeError("回复目标缺少 message_ref。")
            result = workflow.reply_to_email(
                message_ref=str(source_holder["message_ref"]),
                reply_all=False,
                template_ref=str(template_holder["template_ref"]),
                body_html=f"<p>{marker} decoupled formatted reply</p>",
                copy_template_attachments=False,
            )
            if result.get("status") != "draft_created":
                raise RuntimeError(f"reply_to_email 未创建草稿：{result.get('status')}")
            if result.get("sent") is not False:
                raise RuntimeError("模板化回复草稿的 sent 必须为 false。")
            draft = result["draft"]
            created_drafts.append({
                "draft_type": "reply_to_email_template",
                "draft_ref": draft.get("draft_ref"),
                "item_id": draft.get("item_id"),
            })
            return {
        ...
        run_step("reply_to_email_with_template_ref", reply_with_template)
...
def run_dt_suite(
    ...
    *,
    ...
    Default groups:
      ...
      - template-mail-v06: decoupled source-email template extraction (v0.7 architecture)
    ...
    if "template-mail-v06" in normalized_groups:
        resolved_app_config = app_config or getattr(client, "config", None)
        if resolved_app_config is None:
            domain = profile.draft_recipient.rsplit("@", 1)[1]
            resolved_app_config = AppConfig(
        ...
        template = run_template_v06_integration_tests(
            client, profile, app_config=resolved_app_config, read_only=read_only,
            store=store, stamp=stamp,
        )
        group_reports.append({
            "id": "template-mail-v06",
            "label": "Decoupled Email Template DT",
            "summary": template["summary"],
            "steps": template["steps"],
            "created_drafts": template.get("created_drafts") or [],
        })
